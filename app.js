...
function setupEventListeners() {
    ...
    const btnImportEntidades = document.getElementById('btn-import-entidades');
    if (btnImportEntidades) {
        btnImportEntidades.addEventListener('click', () => document.getElementById('file-import-entidades').click());
        document.getElementById('file-import-entidades').addEventListener('change', handleImportEntidades);
    }

    const btnImportEvaluadores = document.getElementById('btn-import-evaluadores');
    if (btnImportEvaluadores) {
        btnImportEvaluadores.addEventListener('click', () => document.getElementById('file-import-evaluadores').click());
        document.getElementById('file-import-evaluadores').addEventListener('change', handleImportEvaluadores);
    }
}
...
/* ================= IMPORTADORES EXCEL ================= */
function handleImportEntidades(e) {
    const file = e.target.files[0];
    if(!file) return;
    
    showProgressBar("Importando Entidades...");
    let progress = 0;
    const interval = setInterval(() => { progress += 10; if (progress > 80) progress = 80; updateProgressBar(progress); }, 50);

    const reader = new FileReader();
    reader.onload = function(evt) {
        const buffer = evt.target.result; // Capturar en memoria antes de la asincronía
        setTimeout(() => {
            try {
                ...
                let count = 0;
                rows.forEach((row, i) => {
                    if (i === 0 || !row || !row[0]) return; // Validación segura de celdas vacías
                    const rut = String(row[0]).trim();
                    const nombre = String(row[1] || '').trim();
                    if(!rut || !nombre) return;
                    
                    const idEntidad = 'ent_' + Date.now().toString() + '_' + Math.floor(Math.random() * 1000) + i;
                    const comuna = String(row[2] || '').trim();
                    const programa = String(row[3] || '').trim();
                    const convenio = String(row[4] || '').trim();
                    const fecha = String(row[5] || '').trim();
                    
                    tx.objectStore('entidades').put({ idEntidad, rut, nombre, comuna, programa, convenio, fecha });
                    count++;
                });
                tx.oncomplete = () => {
                    clearInterval(interval);
                    ...
                    setTimeout(() => {
                        hideProgressBar();
                        if (count === 0) {
                            alert('Atención: No se detectaron datos válidos. Verifique que la Columna A tenga el RUT y los datos comiencen en la Fila 2.');
                        } else {
                            alert(`Se importaron ${count} entidades exitosamente.`);
                        }
                        populateAdminMatrix();
                        e.target.value = ''; // FIX SEGURO: Limpiar al finalizar
                    }, 300);
                };
            } catch (error) {
                clearInterval(interval);
                hideProgressBar();
                alert('Error crítico al leer el archivo. Asegúrese de que el documento no esté corrupto y tenga formato Excel (.xlsx).');
                e.target.value = '';
                console.error('Error procesando Excel:', error);
            }
        }, 100);
    };
    reader.onerror = () => { clearInterval(interval); hideProgressBar(); alert('Error de lectura en el navegador.'); e.target.value = ''; };
    reader.readAsArrayBuffer(file);
}

function handleImportEvaluadores(e) {
    const file = e.target.files[0];
    if(!file) return;

    showProgressBar("Importando Evaluadores...");
    let progress = 0;
    const interval = setInterval(() => { progress += 10; if (progress > 80) progress = 80; updateProgressBar(progress); }, 50);
        ...
        setTimeout(() => {
            try {
                ...
                rows.forEach((row, i) => {
                    ...
                    const clave = String(row[3] || '123456').trim();
                ...
                tx.oncomplete = () => {
                    ...
                    setTimeout(() => {
                        hideProgressBar();
                        if (count === 0) {
                            alert('Atención: No se detectaron evaluadores válidos. Verifique que la Columna A tenga el RUT y los datos comiencen en la Fila 2.');
                        } else {
                            alert(`Se importaron ${count} evaluadores exitosamente.`);
                        }
                        populateAdminMatrix();
                        e.target.value = ''; // FIX SEGURO: Limpiar al finalizar
                    }, 300);
